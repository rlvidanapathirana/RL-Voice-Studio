...
function resolve_config_root(root, current_location) {
...
function submit(endpoint, data = {}, event_data, trigger_id, all_events, additional_headers) {
	try {
		...
		const job = this.handle_blob(config.root, resolved_data, endpoint_info).then(async (_payload) => {
			payload = {
				data: handle_payload(_payload, dependency, config.components, "input", true) || [],
				event_data,
				fn_index,
				trigger_id,
	...
	async _resolve_heartbeat(_config) {
		if (_config) {
			...
			if (this.config && this.config.connect_heartbeat) {
				if (this.config.space_id && this.options.token) this.jwt = await get_jwt(this.config.space_id, this.options.token, this.cookies);
			}
		}
		if (_config.space_id && this.options.token) this.jwt = await get_jwt(_config.space_id, this.options.token);
		if (this.config && this.config.connect_heartbeat) {
...
//#endregion
export { Client, FileData, MISSING_CREDENTIALS_MSG, client, duplicate_space as duplicate, handle_file, predict, prepare_files, submit, upload, upload_files };
